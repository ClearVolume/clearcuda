...
public class CudaDevicePointer extends CopyFromToInterface implements
		CudaCloseable, PeerInterface<CUdeviceptr> {
	...
	public static final CudaDevicePointer mallocManaged(final long pSizeInBytes) {
		...
		cuMemAllocManaged(lCudaDevicePointer.getPeer(), pSizeInBytes, lFlags);
		return lCudaDevicePointer;
	}

	CudaDevicePointer(boolean pExternallyAllocated, final long pSizeInBytes) {
		super();
	...
	public void setFloat(float pFloat) {
		copyFrom(Pointer.to(new float[] { pFloat }), true);
	}

	public void setDouble(double pDouble) {
		copyFrom(Pointer.to(new double[] { pDouble }), true);
	}
	...
	public void setLong(long pLong) {
		copyFrom(Pointer.to(new long[] { pLong }), true);
	}
	...
	public void copyFrom(CudaDevicePointer pCudaDevicePointer, boolean pSync) {
		cuMemcpyAsync(mCUdeviceptr, pCudaDevicePointer.getPeer(),
				min(getSizeInBytes(), pCudaDevicePointer.getSizeInBytes()),
				null);
		if (pSync)
			cuCtxSynchronize();
	}

	public void copyTo(CudaDevicePointer pCudaDevicePointer, boolean pSync) {
		cuMemcpyAsync(pCudaDevicePointer.getPeer(), mCUdeviceptr,
				min(getSizeInBytes(), pCudaDevicePointer.getSizeInBytes()),
				null);
		if (pSync)
			cuCtxSynchronize();
